...
import {Verifier as TakeOrderVerifier} from "./verifiers/TakeOrderVerifier.sol";
import "./ZkDaiBase.sol";


contract TakeOrder is TakeOrderVerifier, ZkDaiBase {
  ...
  */
  function takeOrderCommit(bytes32 proofHash)
    internal
  {
      ...
      require(notes[_notes[3]] == State.Invalid, "output note3 is already minted");

      notes[_notes[0]] = State.Spent;
      notes[_notes[1]] = State.Committed;
      notes[_notes[2]] = State.Committed;
      notes[_notes[3]] = State.Committed;
      ...
      emit NoteStateChange(_notes[3], State.Committed);
  }

  function get4Notes(uint256[] input)
    internal
    pure
