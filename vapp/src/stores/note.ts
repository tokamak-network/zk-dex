...
export interface Note {
  hash: string
  owner: string
  owner0?: string  // BabyJubJub public key X
  owner1?: string  // BabyJubJub public key Y
  value: string
...
export const useNoteStore = defineStore('note', () => {
  ...
  /**
   * Compute note hash from note data (matching contract's calcHash)
   */
  function computeNoteHash(owner0: string, owner1: string, value: string, token: string, viewingKey: string, salt: string): string {
    const abiCoder = new AbiCoder()
    const encoded = abiCoder.encode(
      ['uint256', 'uint256', 'uint256', 'uint256', 'uint256', 'uint256'],
      [owner0, owner1, value, token, viewingKey, salt]
    )
        ...
        try {
          ...
          for (const account of accounts) {
            if (account.publicKey && isNoteOwner(decoded, account.publicKey)) {
              // Verify hash matches
              const computedHash = computeNoteHash(
                decoded.owner0,
                decoded.owner1,
                decoded.value,
                decoded.token,
              ...
              // Note hash should match (allowing for potential format differences)
              const note: Note = {
                hash: noteHash,
                owner: account.address,
                owner0: decoded.owner0,
                owner1: decoded.owner1,
                value: decoded.value,
                token: decoded.token,
