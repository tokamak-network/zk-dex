...
function getCreateNoteParams(_pubKey, _value, _nonce) {
  ...
  const pubKey = new BN(_pubKey, 16).toString(16); // 32 bytes = 256 bits
  const value = new BN(_value, 16).mul(SCALING_FACTOR).toString(16, 32); // 16 bytes = 128 bits
  const nonce = new BN(_nonce, 16).toString(16, 32); // 16 bytes = 128 bits
  const privateParams = [pubKey.slice(0, 32), pubKey.slice(32), nonce];

  const note = pubKey + value + nonce;
  console.log('note', note);

  const publicParams = getNoteHash(note).concat(value);
