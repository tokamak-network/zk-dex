...
function getNoteParams(_key, _val, _nonce) {
  const key = new BN(_key, 16).toString(16); // 32 bytes = 256 bits
  const val = new BN(_val, 16).mul(SCALING_FACTOR).toString(16, 32); // 16 bytes = 128 bits
  const nonce = new BN(_nonce, 16).toString(16, 32);
...
function getCreateNoteParams(_sender, _value, _nonce, _receiver, _val, _n0, _n1) {
  const oldNote = getNoteParams(_sender, _value, _nonce);
  const newNote0 = getNoteParams(_receiver, _val, _n0);
  const newNote1 = getNoteParams(_sender, new BN(_value, 16).sub(new BN(_val, 16)).toString(16), _n1); // leftover change
