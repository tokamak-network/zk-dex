...
function getTransferCmd(
  from0Owner, from0Value, from0Type, from0ViewKey, from0Salt,
  ...
  const from0Params = noteHelper.getNoteParams(from0Owner, from0Value, from0Type, from0ViewKey, from0Salt);
...
function getConvertCmd(
  smartOwner, smartValue, smartType, smartViewKey, smartSalt,
  originOwner, originValue, originType, originViewKey, originSalt,
  noteOwner, noteValue, noteType, noteViewKey, noteSalt,
  sk,
) {
  const smartParams = noteHelper.getNoteParams(smartOwner, smartValue, smartType, smartViewKey, smartSalt);
...
function getTakeOrderCmd(
  parentOwner, parentValue, parentType, parentViewKey, parentSalt, // parent note's variables
  ...
  const parentNoteParams = noteHelper.getNoteParams(parentOwner, parentValue, parentType, parentViewKey, parentSalt);
...
function getSettleOrderCmd(
  makerNoteOwner, makerNoteValue, makerNoteType, makerNoteViewKey, makerNoteSalt, // makerNote's variables
  ...
  const makerNoteParams = noteHelper.getNoteParams(makerNoteOwner, makerNoteValue, makerNoteType, makerNoteViewKey, makerNoteSalt);
